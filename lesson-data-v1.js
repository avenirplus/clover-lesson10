...
window.LESSON_DATA=src.map(x=>{
 ...
 const chunks=Array.isArray(phr.en)&&phr.en.length?phr.en:(x.output?.phrases||[x.completed]).filter(Boolean);
 const jaChunks=Array.isArray(phr.ja)&&phr.ja.length===chunks.length?phr.ja:[x.output?.jp||x.translation||''].filter(Boolean);
 ...
 correct:x.why||[],wrong:x.wrong||[],method:x.decision||'',
 outputCue:jaChunks.join(' / '),outputChunks:chunks,outputJaChunks:jaChunks,outputNaturalJa:naturalJa,
 outputDefault:Object.freeze({en:[...chunks],ja:[...jaChunks],naturalJa}),
 tokens:x.tokens||[],acceptedAnswers:x.acceptedAnswers||[],
